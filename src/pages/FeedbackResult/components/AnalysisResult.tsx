...
export default function AnalysisResult({ data }: { data?: any }) {
  const progressData = [
    { label: '발화 속도', score: data?.speechScore ?? 0 },
    { label: '시선 집중도', score: data?.gazeScore ?? 0 },
    { label: '주제 적절성', score: data?.topicScore ?? 0 },
    { label: '제스처 안정성', score: data?.postureScore ?? 0 },
  ]
  ...
  return (
    <div className="min-h-screen w-full bg-[#F7F7F8]">
      ...
      <div className="mx-auto w-full max-w-4xl px-6 pt-16">
        ...
        <div className="mt-11">
          <LinearProgressBar label="추임새 점수" score={data?.fillerScore ?? 0} maxScore={100} />
        </div>
      </div>
        ...
        <div className="mx-auto w-full max-w-4xl px-6 pb-10 pt-12 leading-6 text-[#3B3B3B]">
          ...
          <div className="mb-10 border-b-2 border-[#D7D6F1] pb-10">
            <p className="fontBold text-[17px]">
              <span className="text-[#5650FF]">{userName}</span> 님은 이런 점이 좋았어요!
            </p>
            <p className="mt-6 whitespace-pre-line px-4 text-[13px]">{goodPoints}</p>
          </div>

          {/* 성장 포인트 */}
          <div className="mb-10 border-b-2 border-[#D7D6F1] pb-10">
            <p className="fontBold text-[17px]">
              조금 더 <span className="text-[#5650FF]">성장할 수 있는 포인트</span>예요!
            </p>
            <p className="mt-6 whitespace-pre-line px-4 text-[13px]">{growthPoints}</p>
          </div>
          ...
          <div className="mb-10 pb-10">
            ...
            {practicePoints && (
              <p className="mt-4 whitespace-pre-line px-4 text-[13px] text-gray-600">
                {practicePoints}
              </p>
            )}
            <div className="mt-6 px-4">
              <div className="mt-8 flex flex-col gap-8 md:flex-row">
                <div className="flex h-32 w-full cursor-pointer items-center rounded-3xl border-2 border-[#FFA956]/[.56] bg-white px-6 transition-all duration-300 hover:scale-105 hover:shadow-lg md:w-[50%]">
                ...
                <div className="flex h-32 w-full cursor-pointer items-center rounded-3xl border-2 border-[#FFA956]/[.56] bg-[#FFA956] px-6 transition-all duration-300 hover:scale-105 hover:shadow-lg md:w-[50%]">
                  <div className="flex basis-[30%] justify-end">
                    <img
                      src={IMAGES.speakPractice}
                      alt="시선 및 표정 훈련 아이콘"
